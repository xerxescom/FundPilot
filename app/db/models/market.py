...
class MarketValuationDaily(Base):
    __tablename__ = "market_valuation_daily"
    __table_args__ = (UniqueConstraint("index_code", "trade_date", name="uq_market_valuation_date"),)
    ...
    pe_percentile: Mapped[Decimal | None] = mapped_column(Numeric(8, 4))
    source: Mapped[str | None] = mapped_column(String(50))
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
