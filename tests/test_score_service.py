...
def test_score_payload_high_pe_percentile_weakens_market_signal(db_session):
    ...
    assert payload["valuation_index_name"] == "沪深300"
    assert payload["buy_window_signal"] != "favorable"
    assert "PE 百分位偏高" in payload["market_reason"]
    assert "market_weak" in payload["risk_flags"]


def test_score_payload_uses_matched_index_pe_not_market_average(db_session):
    today = date.today()
    db_session.add_all(
        [
            FundInfo(
            ...
            MarketIndexDaily(index_code="sh000905", index_name="中证500", trade_date=today, close=Decimal("100")),
            MarketValuationDaily(
                index_code="sh000300",
                index_name="沪深300",
                trade_date=today,
