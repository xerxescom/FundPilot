...
Vue 前端位于 `frontend/`，是 FundPilot 的唯一前端工作台。所有用户交互都优先在 Vue 中完成，后端通过 `/api/v1` 提供结构化数据。
...
1. 启动 FastAPI 和 Vue。
2. 打开今日驾驶舱，确认待办、风险、数据状态、市场概览和预警可读。
3. 执行完整链路：添加自选 -> 一键同步并分析 -> 基金详情 -> 评分排行 -> 风险预警 -> AI 简报。
4. 打开持仓页，录入买入记录，确认持仓汇总、持仓占比和组合诊断正常。
